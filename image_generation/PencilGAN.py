...
"""
@File    :   PencilGAN.py
@Time    :   2024/02/24 22:49:38
@Programme :  MSc Integrated Machine Learning Systems (TMSIMLSSYS01)
@Module : ELEC0135: Applied Machine Learning Systems II
@SN :   23043574
@Contact :   uceewl4@ucl.ac.uk
@Desc    :  This file includes all procedures for pencil GAN.
...
class ModelMonitor(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if not os.path.exists("outputs/image_generation/pencilGAN"):
            os.makedirs("outputs/image_generation/pencilGAN")
        for i in range(self.num_img):
            img = array_to_img(generated_images[i])
            if (epoch in range(0, 800, 10)) or (epoch in range(0, 10)):
                img.save(
                    f"outputs/image_generation/pencilGAN/generated_img_{epoch}_{i}.png"
                )
            else:
                pass
...
class PencilGAN(Model):
    ...
    def train(self, model, Xtrain):
        ...
        self.discriminator.save(
            "outputs/image_generation/models/pecilGAN_disciriminator.h5"
        )
    ...
    def generate(self):
        ...
        self.generator.load_weights(
            "outputs/image_generation/models/pencilGAN_generator.h5"
        )
        generated_images = self.generator.predict(tf.random.normal((100, 64, 1))) * 255
        if not os.path.exists("outputs/image_generation/pencilGAN/"):
            os.makedirs("outputs/image_generation/pencilGAN/")
        for index, image in enumerate(generated_images):
            cv2.imwrite(f"outputs/image_generation/pencilGAN/img_{index}.JPG", image)
