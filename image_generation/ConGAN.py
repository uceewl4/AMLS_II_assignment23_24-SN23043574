...
class ConGAN(Model):
    ...
    def genNet(self):  # input 100 100 3
        ...
        gen = Dense(25*25*128)(input) # begin with 128 samples into 7*7*128
        gen = LeakyReLU(0.2)(gen)
        gen = Reshape((25,25,128))
        ...
        output = Conv2D(3, 4, padding='same', activation='sigmoid')(gen) # 3 channel
    ...
    def disNet(self): 
        ...
        merge = Concatenate()([input, label])
       
        # First Conv Block
        dis = Conv2D(32, 5, input_shape = (100,100,3))(merge)
    ...
    def train_step(self,train_batch, label_batch):  # override the train_step method in fit
        # Get the data 
        real_image = train_batch
        real_label = label_batch
        fake_label = np.random.randint(0,12,self.batch_size)
        fake_image = self.generator([np.random.normal((self.batch_size, 128, 1)),\
  ...
  """
    ...
    def generate(self):
        ...
        generated_images = self.generator.predict([np.random.normal((100,128,1)),np.random.randint(0,12,self.batch_size)])
