...
def data_preprocess():
    ...
    for name in ["train", "val", "test"]:
        ...
        # os.makedirs(f"/scratch/uceewl4/preprocessed500/{name}", exist_ok=True)
        for index, f in enumerate(os.listdir(raw_path)):
            if not os.path.isfile(os.path.join(raw_path, f)):
                ...
                cv2.imwrite(
                    ...
                    # os.path.join(f"/scratch/uceewl4/preprocessed500/{name}", f"{f}"), imgPas
                )
...
def image_pointcloud(pre_data):
    if pre_data == True:
        ...
        for name in ["train", "val", "test"]:
            ...
            for index, f in enumerate(os.listdir(pre_path)):
                if not os.path.isfile(os.path.join(pre_path, f)):
                    ...
                    fig.savefig(os.path.join(f"/scratch/uceewl4/pc/pc_visual_1/{name}", f"{f}"))
                    plt.close()
                    ...
                    fig_plotly.write_html(os.path.join(f"/scratch/uceewl4/pc/pc_visual_2/{name}", f"{f[:-4]}.html"))
                    plt.close()
                feature_dict[f"{f}"] = {}
        ...
        pass
# image_pointcloud(pre_data=True)
...
def image_augmentation(pre_data=False):
    if pre_data == True:
        for name in ["train", "val", "test"]:
            ...
            aug_index = random.sample([i for i in range(6000)],300) if name == "train" else \
                random.sample([i for i in range(2400)],100)
            for i in aug_index:
        ...
        pass
# image_augmentation(pre_data=True)